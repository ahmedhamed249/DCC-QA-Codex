        ...
        <tr class="border-b border-slate-100 dark:border-slate-800">
          ...
          <td class="px-2 py-2">
            <div class="flex gap-2">
              <button class="rounded-lg border border-slate-300 px-2 py-1 text-xs dark:border-slate-700" data-edit-audit="${audit.id}">Edit</button>
              <button class="rounded-lg border border-rose-300 px-2 py-1 text-xs text-rose-700 dark:border-rose-800 dark:text-rose-300" data-delete-audit="${audit.id}">Delete</button>
            </div>
          </td>
        </tr>
      `,
  ...
  el.handlerAuditList.innerHTML = mine
    .map(
      ...
      <article class="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
        ...
        <button class="mt-3 rounded-lg border border-slate-300 px-3 py-1 text-sm dark:border-slate-700" data-appeal-audit="${audit.id}">Appeal Audit</button>
    ...
    button.addEventListener("click", () => {
      ...
      wrap.innerHTML = `
        <form class="mt-3 space-y-2" data-appeal-form="${button.dataset.appealAudit}">
          ...
          <button class="rounded-lg bg-brand-600 px-3 py-1 text-sm font-semibold text-white">Submit Appeal</button>
        </form>
      `;
...
function wireEvents() {
  ...
  el.loginForm.addEventListener("submit", async (event) => {
    ...
    el.authState.textContent = "Signing in...";
    ...
    try {
      ...
      el.authState.textContent = `Login failed: ${error.message}`;
    }
  });
